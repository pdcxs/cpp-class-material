...
int main() {
  cout << "Please input ring size and step size.\n"
       << "Input a blank line will exit this program" << endl;
...
bool parseLine() {
  ...
  if (stepSize < 1) {
    cout << "Step size should be an positive number" << endl;
    return true;
  }

  optional<josephus::Ring> ring = josephus::create(ringSize);
  if (!ring.has_value()) {
    cout << "Cannot create a Josephus ring with " << ringSize << " elements."
         << endl;
    return true;
  }
...
optional<pair<int, int>> getTwoNums(const string &s) {
  ...
  if (nums.size() != 2) {
    cout << "Need 2 numbers, but you provide " << nums.size() << " number";
    cout << (nums.size() <= 1 ? "." : "s.") << endl;
    return nullopt;
  }

  optional<int> num1 = toInt(nums[0]);
  if (!num1.has_value()) {
    cout << "Cannot convert " << nums[0] << " to an integer." << endl;
    return nullopt;
  }

  optional<int> num2 = toInt(nums[1]);
  if (!num2.has_value()) {
    cout << "Cannot convert " << nums[1] << " to an integer." << endl;
    return nullopt;
  }
